...
const isSet = <T extends Record<string, any>>(def: T | undefined, key: string): def is T => Object.keys(def ?? {}).includes(key);

export const omitUnsetKeys = <T>(defaults: Record<string, unknown>, def?: Record<string, unknown>): T => {
...
export function object(properties: ObjectMapping['properties'], def?: WithoutTypeField<ObjectMapping>): ObjectMapping {
