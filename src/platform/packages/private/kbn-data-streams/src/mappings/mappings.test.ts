...
describe('omitUnsetKeys', () => {
  it('omits undefined keys', () => {
    ...
    expect(omitUnsetKeys({ a: 1, b: 2 }, { a: undefined, c: undefined, d: 2 })).toEqual({ b: 2, d: 2 });
    
    expect(omitUnsetKeys({ a: 1, removed_nested: { a : 1} }, { removed_nested: undefined })).toEqual({ a: 1 });
    expect(omitUnsetKeys({ a: 1, updated_nested: { a : 1 } }, { updated_nested: { b: 3 } })).toEqual({ a: 1, updated_nested: { b: 3 } });
  });
});